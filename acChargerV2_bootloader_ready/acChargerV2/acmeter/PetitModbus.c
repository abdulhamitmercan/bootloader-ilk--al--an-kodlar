...
unsigned char CheckPetitModbusBufferComplete(void)
{
    ...
    if(PetitReceiveCounter>4)
    {
        if(PetitReceiveBuffer[0]==PETITMODBUS_SLAVE_ADDRESS)
        {
            ...
            {
                PetitExpectedReceiveCount    =PetitReceiveBuffer[2] + 5;//8;
            }
...
void modbus_Process(unsigned int *rxRdy)
{
    Petit_RxRTU();                                                              // Call this function every cycle
...
void modbus_readHoldingRegister(unsigned char addr, unsigned char len, unsigned char* data){
    int i = 0;
    Petit_Tx_Data.Function    = PETITMODBUS_READ_HOLDING_REGISTERS;
    Petit_Tx_Data.Address     = addr;
    Petit_Tx_Data.DataLen     = len;

    for(i = 0; i < len; i++){
        Petit_Tx_Data.DataBuf[i] = *(data+i);
    }
...
void modbus_getData(uint32_t* data){
    uint32_t dataCpy[4] = {0};
    dataCpy[0] = Petit_Rx_Data.DataBuf[1];
    dataCpy[1] = Petit_Rx_Data.DataBuf[2];
    dataCpy[2] = Petit_Rx_Data.DataBuf[3];
    dataCpy[3] = Petit_Rx_Data.DataBuf[4];

    *data = dataCpy[0]<<24 | dataCpy[1]<<16
            | dataCpy[2]<<8 | dataCpy[3];
